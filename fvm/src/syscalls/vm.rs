...
use super::*;
use crate::{kernel::{ClassifyResult, Kernel, NetworkOps, MessageOps}, BaseKernel};
...
pub(super) struct Vm<K> (PhantomData<K>);

// TODO this is contrained to include NetworkOps which i would rather avoid, 
impl<K> Vm<K> {
...
impl InvokeContext {
    pub fn context<K: NetworkOps + MessageOps>(context: Context<'_, K>) -> crate::kernel::Result<InvocationContext> {
...
impl<K: BaseKernel + NetworkOps> Bind<K, VmAbort> for K {
    fn bind_syscalls(linker: &mut Linker<InvocationData<K>>) -> anyhow::Result<()> {
        linker.bind("vm", "abort", VmAbort::abort)?;
        Ok(())
    }
}

impl<K: BaseKernel + NetworkOps + MessageOps> Bind<K, InvokeContext> for K {
    fn bind_syscalls(linker: &mut Linker<InvocationData<K>>) -> anyhow::Result<()> {
        linker.bind("vm", "context", InvokeContext::context)?;
        Ok(())
    }
}
